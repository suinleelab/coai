...
#==============================
class Environment:
    def __init__(self, data, hpc_p, costs, groups=None):
        ...
        self.groups = np.arange(self.costs.shape[0]) if groups is None else groups
    ...
    def step(self, action):
        ...
        # Need this to handle the multiple-agents-at-once framework
        action_f = np.clip(action - config.TERMINAL_ACTIONS, 0, config.FEATURE_DIM)
#         action_groups = self.groups[action_]
        self.mask[self.groups[action_f].reshape(-1,1)==self.groups.reshape(1,-1)]=1
#         for i in range(config.FEATURE_DIM):
#             if self.groups[i]==self.groups[action_f]:
#                 self.mask[lin_array, i] = 1
